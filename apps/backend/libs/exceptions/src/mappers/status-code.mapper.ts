...
export class StatusCodeMapper {
  static readonly statusCodeStrategy: Record<status, HttpStatus> = {
  ...
  static fromGrpcToHttp(code: status): HttpStatus {
    return this.statusCodeStrategy[code] ?? HttpStatus.INTERNAL_SERVER_ERROR;
  }

  static fromHttpToGrpc(code: HttpStatus): status {
    for (const [grpcStatus, httpStatus] of Object.entries(this.statusCodeStrategy)) {
      if (httpStatus === code) {
        return grpcStatus as unknown as status;
      }
    }
